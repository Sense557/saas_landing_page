                    ...
                    <div
                        ...
                        {features.map(({id, icon, caption, title, text, button}) => (
                            <div key={id}
                                ...
                                <h2 className={"max-w-400 h3 max-md:h5 max-md:mb-6 mb-7 text-p4 "}>{title}</h2>
                            ...
                            {details.map(({id, icon, title}) => (
                                <li key={id} className={"relative pt-16 px-4 pb-14"}>
                                    ...
                                    <div className={"flex items-center justify-center mx-auto mb-3 border-2 border-s2 rounded-full hover:border-s4 transition-all duration-500 size-20 shadow-500"}>
                                        <img src={icon} className={"size-17/20 z-20 object-contain"} alt={title}/>
                                    </div>
                                    <h3 className={"relative max-w-36 z-2 mx-auto my-0 base-small text-center uppercase"}>{title}</h3>
                                </li>
                            ))}
                        </ul>
                    </div>
